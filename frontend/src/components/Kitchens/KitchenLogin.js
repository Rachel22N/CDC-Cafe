...
function KitchenLogin() {
  ...
  const handleLoginConfirmClick = () => {
    ...
    fetch('http://localhost:5000/kitchenloginpage', {
    method: 'POST',
    headers: {
        'Content-Type': 'application/json',
    },
    body: JSON.stringify({
      username: document.getElementById('kitchen-staff-email').value,
      password: document.getElementById('kitchen-staff-password').value,
      role: 'Kitchen'
    }),
    })
    .then(response => {
